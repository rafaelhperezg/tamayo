...
# /--------------end TREASURY METHODS---------------
   ...
   def hyper_method
     ...
     self.current_employees       = @enterprise.current_number_of_employees #method
     self.current_prod_capacity   = @enterprise.today_workshop_production_capacity #method
     previous_backlog                   = @enterprise.current_backlog # column
     self.current_to_produce      = @enterprise.total_to_produce_today(today_orders_received, @enterprise.current_backlog) #method
     self.est_manufactured_today  = @enterprise.products_manufactured_today(today_workshop_production_capacity, total_to_produce_today) # method
     self.est_delivery_time       = @enterprise.when_can_todays_orders_be_delivered(@today_workshop_production_capacity, @total_to_produce_today)

     # COSTS VARIABLES
     self.current_salaries           = @enterprise.cost_of_salaries_for_today #Method
     self.current_raw_materials      = @enterprise.cost_of_raw_materials_for_today(today_orders_received) #method
     self.current_cost_hiring_firing = @enterprise.cost_of_hiring_and_firing_for_today(@gamedecisions.last.employees_variation)#GAME DECISIONS A CALCULER
     ...
     self.est_total_sales_today      = @enterprise.total_sales_for_today( @products_manufactured_today, @contract.timeframe, @when_can_todays_orders_be_delivered, @contract.price )

     # TREASURY VARIABLES =>Test for the 2 variables: OK
     self.est_net_result_today       = total_sales_for_today - total_money_spent_today
     @prev_current_cash                    = @enterprise.current_cash
     self.current_cash               = @enterprise.total_treasury_today(@net_result_today) #METHOD
